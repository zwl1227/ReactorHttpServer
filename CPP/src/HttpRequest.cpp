...
bool HttpRequest::parseHttpRequestBody(Buffer *buf)
{
    Debug("解析请求数据！");
    setState(ProcessState::ParseReqDone);
    return true;
...
bool HttpRequest::processHttpRequest(HttpResponse *response)
{
    bool flag = false;
    if (m_method == "GET")
        flag = processGetRequest(response);
    else if (m_method == "POST")
        flag = processPostRequest(response);
    return flag;
}
...
bool HttpRequest::processGetRequest(HttpResponse *response)
{
    ...
    if (ret == -1)
    {
        const char *tmp = "api/data";
        if (strncasecmp(file, tmp, strlen(tmp)) == 0)
        {
            Debug("/api/data");
        }
        // 文件不存在 -- 回复404
...
void HttpRequest::sendDir(string dirName, Buffer *sendbuf, int cfd)
{
    ...
    if (!conn->query("SELECT count(*) FROM resume_info"))
    {
        Debug("sql query error!");
        return;
    }
    conn->next();
    int count = stoi(conn->value(0));
    if (!conn->query("SELECT * FROM resume_info limit 0,10"))
    {
        Debug("sql query error!");
        return;
    }
...
void HttpRequest::sendJson(string info, Buffer *sendbuf, int cfd)
{
}
